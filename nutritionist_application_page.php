...
<head>
	...
	<style>
		...
		body {
			...
			color: #333;
			display: flex;
			background-image: url(/pictures/nutrition.jpg);
			background-size: cover;
		}

		.container .nutrtionist .box-container {
			display: flex;
			padding: 70px;
			flex-wrap: wrap;
			gap: 50px;
		...
		.container .nutrtionist .box-container .box {
			text-align: center;
			border-radius: 5px;
			position: relative;
			padding: 20px;
		...
		.container .nutrtionist .box-container .box .text {
			padding: 10px;
		}
...
<body>
	...
	<div class="container">
		<div class="nutrtionist">
			<h1 style="padding-left:500px; color:black;">Nutrtionist</h1>
			<form action="nutritionist_application_page.php" method="post">
				<div id="citySection">
					<h2>Select City:</h2>
				...
				<div>
					...
					<input style="margin-left: 580px; margin-top: 20px;" type="submit" value="View" id="search" class="btn btns">
				</div>
			</form>
			<div class="box-container" id="nutritionistBoxContainer">
				<?php
				if ($_SERVER["REQUEST_METHOD"] == "POST") {
					$city = $_POST['cityDropdown'];

					$sql = "SELECT * FROM `nutritionists_tbl` WHERE city = '$city'";

					$select_nutritionist = mysqli_query($conn, $sql);
					if (mysqli_num_rows($select_nutritionist) > 0) {
						while ($fetch_info = mysqli_fetch_assoc($select_nutritionist)) {
							$image_path = $fetch_info['image'];
				?>
							<div class="box sourceDiv">
								<img src="<?php echo $image_path; ?>" width="120" style="border-radius: 3px;">
								...
								<div class="text">City: <?php echo $fetch_info['city']; ?></div>
								<div class="text">Gender:<?php echo $fetch_info['gender']; ?></div>
								<input type="submit" onclick="showFormContainer(); autoFill(this);" value="Apply" name="Apply" class="btn btns">
							</div>
				<?php
						};
					} else {
						echo "<script>alert('There are no Nutritionist in this Area')</script>";
						echo "<script>window.location='nutritionist_application_page.php';</script>";
					}
				}
				?>
			</div>
			...
			<div id="formContainer" style="display: none;">
				<form action="/insert_applications/nutritionist_requests.php" method="post" enctype="multipart/form-data">
					<br>
					<h2>Payment Form</h2>
					<label for="Nutritionist's First Name">Nutritionist's First Name</label>
					<input type="text" name="Nutritionist's_first_name" class="formInput" readonly>

					<label for="Nutritionist's Last Name">Nutritionist's Last Name</label>
					<input type="text" name="Nutritionist's_last_name" class="formInput" readonly>

					<label for="Nutritionist's Id">Nutritionist's Id</label>
					<input type="text" name="Nutritionist's_id" class="formInput" readonly>
					...
					<label for="file-input" style="color: #04AA6D;">Send Medical Report:</label><br><br>
					...
					<div id="payment" style="display:none;">
						<label for="Fee"> Fee: KSH 8000/-</label><br>
					...
					<div id="request" style="display: none;">
						<input type="submit" class="btn btn-primary" style="margin-left: 110px;" class="btn btns" name="submitButton" value="Send Request"><br><br>
					</div>
				</form>
			</div>
	...
	<script>
			...
			radioButton.addEventListener('change', () => {
				if (radioButton.checked) {
					...
					if (radioButtons[0].checked || radioButtons[1].checked) {
						...
						// Otherwise, hide the payment div
		...
		function autoFill(element) {
			...
			document.querySelector("input[name=\"Nutritionist's_first_name\"]").value = firstName;
			document.querySelector("input[name=\"Nutritionist's_last_name\"]").value = lastName;
			document.querySelector("input[name=\"Nutritionist's_id\"]").value = id;
		};
